...
//                _pathforcheck = store_id + "_COMP_PROMOIMG_" + imgDate + category_id + "_" + brand_id + "_" + sku_id + ".jpg";
//                _path = str + _pathforcheck;
//                startCameraActivity();
//                break;

            /*case R.id.COMP_checkbox:
                if (COMP_checkbox.isChecked()) {
                    if (compltCompetitionList.size() > 0) {
                        if (compltCompetitionList.get(0).isCompIsExist() == false) {
                            compltCompetitionList.clear();
                        }
                    }
            ...
            case -1:
                if (_pathforcheck != null && !_pathforcheck.equals("")) {
                    if (new File((str + _pathforcheck).trim()).exists()) {
                        img1 = _pathforcheck;
                        //  camera.setImageResource(R.drawable.camera_tick_ico);
                        _pathforcheck = "";
                    }
                }
    ...
    public void setcompetitionpromotionDats() {
        ...
        if (compltCompetitionList.size() > 0) {
            // if (compltCompetitionList.get(0).isCompIsExist() == true) {
            // COMP_checkbox.setChecked(true);
