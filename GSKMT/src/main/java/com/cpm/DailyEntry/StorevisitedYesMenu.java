...
public class StorevisitedYesMenu extends Activity {
	...
	GSKMTDatabase db;
	String date, process_id, store_id;
	...
	//ArrayList<TOTBean> beforetotDataHFD = new ArrayList<TOTBean>();
	//ArrayList<TOTBean> beforetotDataWellness = new ArrayList<TOTBean>();
	//ArrayList<TOTBean> beforetotDataOral = new ArrayList<TOTBean>();
...
//		lv = (ListView)findViewById(R.id.list);
		...
		if (totMappingDataHFD.size()>0) {
			//beforetotDataHFD = db.getBeforeTOTData(store_id, "1", process_id);
			afterTOTDataHFD = db.getAfterTOTData(store_id,"1", process_id);
			
			if (/*beforetotDataHFD.size()>0 &&*/ afterTOTDataHFD.size()>0) {
				before_totHFD = true;
				after_totHFD = true;
			}
		} else{
		...
		if (totMappingDataHFD.size()!=0) {
			if (beforeStockDataHFD.size()>0 && afterStockDataHFD.size()>0 /*&& beforetotDataHFD.size()>0*/ && afterTOTDataHFD.size()>0 &&
		...
		if (totMappingDataWellness.size()>0) {
			//beforetotDataWellness = db.getBeforeTOTData(store_id, "2", process_id);
			afterTOTDataWellness = db.getAfterTOTData(store_id,"2", process_id);
			
			if (/*beforetotDataWellness.size()>0 &&*/ afterTOTDataWellness.size()>0) {
		...
		if (totMappingDataWellness.size()!=0) {
			if (beforeStockDataWellness.size()>0 && afterStockDataWellness.size()>0 /*&& beforetotDataWellness.size()>0*/ &&
		...
		if (totMappingDataOral.size()>0) {
			//beforetotDataOral = db.getBeforeTOTData(store_id, "3", process_id);
			afterTOTDataOral = db.getAfterTOTData(store_id,"3", process_id);
			
			if (/*beforetotDataOral.size()>0 &&*/ afterTOTDataOral.size()>0) {
				before_totOral = true;
				after_totOral = true;
			}
		} else{
		...
		if (totMappingDataOral.size()!=0) {
			if (beforeStockDataOralcare.size()>0 && afterStockDataOral.size()>0 /*&& beforetotDataOral.size()>0*/ &&
					afterTOTDataOral.size()>0 &&
			...
			if (beforeStockDataOralcare.size()>0 && afterStockDataOral.size()>0 &&
					beforeaddtionalDataOral.size()>0 && mappingPromotion3.size()>0) {
				cat_oral = true;
			}
...
//					System.out.println(""+ category_list.get(position).getCategory_id());
